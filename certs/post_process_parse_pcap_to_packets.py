...
def log(packet, file):
	...
	for layer in packet:
		...
		if layer.layer_name == 'quic':
			successfulConnectionNum += (layer.get_field('connection_number') or '') + ' '
			quicBytes += layer.get_field('packet_length') + ' '
			longPacketType += longPacketTypes[int(layer.get_field('long_packet_type') or 4)] + ' '
			_frame = layer.get_field('frame') or ''
	...
	line = f'{timestamp},{timestamp_seconds},{packetNum},{connectionCount},{successfulConnectionNum.strip()},{totalBytes},{quicBytes.strip()},{layers.strip()},{longPacketType.strip()},{frame.strip()},{frameType.strip()},{long_short.strip()}'
